...
class Device:
    def __init__(self):
        ...
        self.bluetoothservice = self.bus.get_object('org.yaptb.btkbservice','/org/yaptb/btkbservice')
        self.iface = dbus.Interface(self.bluetoothservice, 'org.yaptb.btkbservice')
    ...
    def send_mouse_input(self):
        self.iface.send_mouse(self.mouse_state)
    ...
    def send_keyboard_input(self):
        ...
        for bit in element:
            bin_str += str(bit)
        keys = [0xA1, 0x01, int(bin_str,2), 0x00] + self.state[4:10]
        self.iface.send_keys(keys)
